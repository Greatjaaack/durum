...
OPERATIONS_HINTS: tuple[tuple[tuple[str, ...], str], ...] = (
    ...
    (
        ("заказ продуктов", "заказать продукты", "order products"),
        "Заказ продуктов: нажмите `📦 Заказ продуктов` или `/order_products`.",
    ),
    (
        ("заказ хозтоваров", "хозтовары", "order supplies"),
        "Заказ хозтоваров: нажмите `🧴 Заказ хозтоваров` или `/order_supplies`.",
    ),
)
